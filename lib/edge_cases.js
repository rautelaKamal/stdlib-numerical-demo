...
/**
* Runs edge case tests for the erf function.
*
* @private
*/
function testErf() {
    ...
    console.log('');
    console.log('  ┌── erf(x) Edge Cases ──────────────────────');
    console.log('  │');

    // erf(0) should be exactly 0
    result = erf(0.0);
    testCase(
        'erf(0) = 0',
        result,
    ...
    // erf(+Infinity) should be exactly 1
    result = erf(PINF);
    testCase(
        'erf(+∞) = 1',
        result,
    ...
    // erf(large positive) should be very close to 1
    result = erf(4.0);
    testCase(
        'erf(4) ≈ 0.99999998...',
        result,
...
/**
* Runs edge case tests for the heaviside function.
*
* @private
*/
function testHeaviside() {
    var result;

    console.log('');
    console.log('  ┌── heaviside(x) Edge Cases ────────────────');
    console.log('  │');

    // heaviside(0) should be 1
    result = heaviside(0.0);
    testCase(
        'heaviside(0) = 1',
        result,
        '1',
    ...
    // heaviside(NaN) should be NaN
    result = heaviside(NaN);
    testCase(
        'heaviside(NaN) = NaN',
        result,
        'NaN',
    ...
    // heaviside(negative) should be 0
    result = heaviside(-5.0);
    testCase(
        'heaviside(-5) = 0',
        result,
        '0',
...
/**
* Runs edge case tests for the lognormal cdf.
*
* @private
*/
function testLognormal() {
    ...
    console.log('');
    console.log('  ┌── lognormal.cdf(x) Edge Cases ────────────');
    console.log('  │');

    // lognormal.cdf(NaN) should be NaN
    result = lognormalCDF(NaN, 0.0, 1.0);
    testCase(
        'lognormal.cdf(NaN) = NaN',
        result,
        'NaN',
        isnan(result)
    );

    // lognormal.cdf(0) should be 0
    result = lognormalCDF(0.0, 0.0, 1.0);
    testCase(
        'lognormal.cdf(0) = 0',
        result,
        '0',
    ...
    // lognormal.cdf(sigma <= 0) should be NaN
    result = lognormalCDF(1.0, 0.0, -1.0);
    testCase(
        'lognormal.cdf(x, mu, -1) = NaN',
        result,
        'NaN',
        isnan(result)
    );
